...
class JoinSupportingQueryBuilderParserEloquent extends QueryBuilderParserEloquent
{
    /**
    ...
    protected function makeQuery(Builder $query, stdClass $rule, $queryCondition = 'AND')
    {
        ...
        } catch (QBPEloquentRuleException $e) {
            return $query;
        }
     ...
     * @param Builder $query
     * @return Builder the query builder object
     *@throws QBPEloquentException when an invalid array is passed.
    ...
    private function buildRequireArrayQuery($subclause, Builder $query)
    {
        if ($subclause['operator'] == 'IN') {
            ...
            if (count($subclause['value']) !== 2) {
                throw new QBPEloquentException($subclause['to_value_column'].
