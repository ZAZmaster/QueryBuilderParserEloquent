...
trait QBPEloquentFunctions
{
    ...
    protected function validateCondition($condition)
    {
        ...
        if ($condition !== 'and' && $condition !== 'or') {
            throw new QBPEloquentException("Condition can only be one of: 'and', 'or'.");
        }
     ...
     * @throws QBPEloquentException if value is not a correct type
     */
    ...
    {
        if ($requireArray && !is_array($value)) {
            throw new QBPEloquentException("Field ($field) should be an array, but it isn't.");
        }
    }
    ...
    {
        if (count($value) !== 1) {
            throw new QBPEloquentException("Field ($field) should not be an array, but it is.");
        }
    ...
    private function decodeJSON($json)
    {
        ...
        if (json_last_error()) {
            throw new QBPEloquentException('JSON parsing threw an error: '.json_last_error_msg());
        }

        if (!is_object($query)) {
            throw new QBPEloquentException('The query is not valid JSON');
        }
    ...
    private function ensureFieldIsAllowed($fields, $field)
    {
        if (is_array($fields) && !in_array($field, $fields)) {
            throw new QBPEloquentException("Field ({$field}) does not exist in fields list");
        }
    }
    ...
    protected function makeQueryWhenArray(Builder $query, stdClass $rule, array $sqlOperator, array $value, $condition)
    {
        ...
        throw new QBPEloquentException('makeQueryWhenArray could not return a value');
    }
     ...
     *
     * @return Builder
     * @throws QBPEloquentException when SQL operator is !null
     */
    protected function makeQueryWhenNull(Builder $query, stdClass $rule, array $sqlOperator, $condition)
    {
        ...
        throw new QBPEloquentException('makeQueryWhenNull was called on an SQL operator that is not null');
    }
     ...
     * @param string $condition
     * @return Builder
     *@throws QBPEloquentException when more then two items given for the between
     * @see makeQueryWhenArray
     */
    private function makeArrayQueryBetween(Builder $query, stdClass $rule, $operator, array $value, $condition)
    {
        if (count($value) !== 2) {
            throw new QBPEloquentException("{$rule->field} should be an array with only two items.");
        }
